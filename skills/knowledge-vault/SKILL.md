...
description: >
  ...
  MDGraph workflow, agent task records, and 7-phase engineering workflow.
  Use this skill whenever the user mentions KnowledgeVault, mdgraph, agent task records,
  research notes, vault organization, note creation/update/archive, or /phase commands.
...
> [!summary] TL;DR
> - Vault root: `MyKnowledgeBase/` with 6 directories: `00_inbox/`, `10_agentTasks/`, `20_research/`, `30_knowledge/`, `90_archive/`
...
> - Engineering phases: Create Task → Research → PRD → Plan → Execution → QA → Sink
...
- Using `/phase-*` commands for structured engineering workflow
...
├── 10_agentTasks/  # Agent task records, timestamped
...
| "Agent task / /phase-* / engineering phases" | `references/engineering-phases.md` |
...
| Agent task records and engineering phase workflow | `planning-with-files` for task plan/findings/progress files |
| Creating/updating/searching vault notes via MDGraph | Direct codebase work that doesn't touch the vault |
| `/phase-create-task`, `/phase-research`, `/phase-plan`, etc. | `deepwork` for heavy coding-only sessions (no vault phases) |