...
Record one route on creation. Mark phases outside the route `N/A`.
...
Verify and Crystallize are mandatory in every route.
The `non-execution` route must never enter Execute.
...
For code-changing routes (`implementation-simple`, `implementation-planned`):
- Implementation and verification must use separate agent invocations.
- The verifier must not be the implementation agent.
- Select agents by capability, not hard-coded alias.

If required independent verification is unavailable:
- Set task status `blocked` and stop mdgraph-loop.
- Do not mark Verify complete under a weakened policy.
- Any user-authorized continuation outside this workflow must not set this task to `done`.

For the `non-execution` route this policy does not apply (no code is written). Verification follows the same capability-based selection if subagent review is needed.

## Crystallize

Crystallize always completes, but long-term knowledge creation is optional.
- Write crystallization result to `## Result` in `progress.md`.
- If reusable knowledge exists, update or create a relevant note under `30_knowledge/`.
- Otherwise record `Knowledge crystallized: none`.
- Never create or modify workflow state files (`progress.md`, `findings.md`, `task_plan.md`) outside the vault. Never modify vault files outside this workflow.
- Mark Crystallize `complete` with date. Phase remains `Crystallize` — no next-phase advance. Record result and links. Set task status `done`.

## Persistence

After each phase transition, persist workflow state to `progress.md` and any other task files that changed. Update `updated` frontmatter timestamp on every note that is modified. Task files live exclusively in the vault under `10_tasks/`. Code changes live in the repo — do not write code or config files inside the vault.
